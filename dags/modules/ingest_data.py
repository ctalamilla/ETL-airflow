...
def find_data(company):
    end_point = (
        f"{BASE_URL}?function={STOCK_FN}&symbol={stock_symbol.get(company)}&interval={INTERVAL}&outputsize=full"
        f"&apikey={np.random.choice(APIKEYS, 1)[0]}&datatype=json"
    )
...
def ingest_data_(company):
    ...
    df = (
        pd.DataFrame(data.get("Time Series (30min)"))
        .T.reset_index()
        .rename(
            columns={
                "index": "fecha",
    ...
    df = df.loc[
        df["fecha"]
        .apply(lambda x: dt.strptime(x, "%Y-%m-%d %H:%M:%S"))
        .apply(lambda x: x >= dt.now() - timedelta(days=7))
    ]
