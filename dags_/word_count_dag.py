...
default_args = {'owner': 'mutt', 'retries': 0, 'start_date': datetime(2020, 11, 2)}
with DAG('word_count', default_args=default_args, schedule_interval='0 0 * * *') as dag:
    ...
    spark_job = DockerOperator(
        ...
        command=f'/spark/bin/spark-submit --master local[*] /spark-job/{SPARK_SCRIPT}',
        docker_url='unix://var/run/docker.sock',
        network_mode='bridge',
        do_xcom_push=False,
    )
    insert_file_to_sql = PythonOperator(
        task_id='file_to_sql', python_callable=_insert_file_to_sql
    )
